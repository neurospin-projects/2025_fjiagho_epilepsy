...
def regions(sub):
    graph_pats = [
    'sourcedata/%(sub)s/t1mri/T1pre_*/default_analysis/folds/3.1/%(side)s%(sub)s.arg',
    # 'sourcedata/%(sub)s/t1mri/FreesurferAtlaspre_*/default_analysis/folds/3.1/%(side)s%(sub)s.arg',
]
    ...
    # read sulcal graphs
    for graph_pat in graph_pats:
        lgraph_files = glob.glob(
            f'{f_tract}/{graph_pat}' % {'sub': sub, 'side': 'L'})
        rgraph_files = glob.glob(
            f'{f_tract}/{graph_pat}' % {'sub': sub, 'side': 'R'})
        if len(lgraph_files) != 0 and len(rgraph_files) != 0:
            print('using pattern:', graph_pat)
            lgraph_file = lgraph_files[0]
            rgraph_file = rgraph_files[0]
            break
        else:
            return {}

    l_graph = aims.read(lgraph_file)
    r_graph = aims.read(rgraph_file)
...
for i in range (len(error_PEPR)):
    ...
    if i in Index_No_Position:
        ...
        chamfer.append(-2)
        SVM.append(-2)
    elif i==34 or i==294 or i==625 or i==626 or i==750:
        entropy.append(-2)
        chamfer.append(-2)
        SVM.append(-2)
    else:
